...
namespace TheSleepyKoala.Map
{
    public class MapManager : MonoBehaviour
    {
        ...
        [field: Header("Tiles")]
        [field: SerializeField] public TileBase[] FloorTile = new TileBase[0];
        [field: SerializeField] public TileBase[] WallTile = new TileBase[0];
        ...
        public void UpdateFogMap(List<Vector3Int> playerFOV)
        {
            ...
            {
                if (State.tiles.ContainsKey((Vector3)pos))
                {
        ...
        public void LoadState(MapState savedState)
        {
            ...
            {
                Vector3Int posInt = new Vector3Int((int)pos.x, (int)pos.y, (int)pos.z);
                if (State.tiles[pos].name.Contains("floor"))
                {
                    FloorMap.SetTile(posInt, FloorTile[Random.Range(0, FloorTile.Length)]);
                }
                else if (State.tiles[pos].name.Contains("wall"))
                {
                    ObstacleMap.SetTile(posInt, WallTile[Random.Range(0, WallTile.Length)]);
                }
